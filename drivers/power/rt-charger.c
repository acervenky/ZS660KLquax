				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			BTM_a2c_cable = false;
			pr_info("%s tcpc_pd_state = PD_CONNECT_PE_READY_SNK/PD3.0, USB_COMM = %d, BTM_a2c_cable = %d\n", __func__, dpm_flags &= DPM_FLAGS_PARTNER_USB_COMM, BTM_a2c_cable);
			if (dpm_flags &= DPM_FLAGS_PARTNER_USB_COMM)
				extcon_set_state_sync(smbchg_dev->extcon, EXTCON_USB, true);
			break;
		case PD_CONNECT_PE_READY_SNK_APDO:
			BTM_a2c_cable = false;
			pr_info("%s tcpc_pd_state = PD_CONNECT_PE_READY_SNK_APDO, BTM_a2c_cable = %d\n", __func__, BTM_a2c_cable);
			rt_chg_get_vid();
			if(rt_chg_check_asus_vid()){
		...
		case PD_CONNECT_PE_READY_SRC_PD30:
			...
			ret = tcpm_inquire_pd_partner_inform(tcpc, vdos);
			vid = vdos[0] & 0xFFFF;
			pr_info("%s tcpc_pd_state = PD_CONNECT_PE_READY_SRC, VID = %x, ret = %d\n",__func__, vid, ret);
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (tcpc_typec_state == TYPEC_UNATTACHED) {
			...
			pr_info("%s send EXTCON_USB = false\n", __func__);
...
static void close5v_by_suspend(struct work_struct *work) {
	...
	if(!isCharging() && (gamevice_active || (pro_dock_active_bottom && (get_net_status() == 0) && (get_prodock_state() ==1)))){
...
void rt_send_screen_suspend(void){
	...
	if((get_DT_state() >= 1) || (get_prodock_state() >=1) || gamevice_active){
