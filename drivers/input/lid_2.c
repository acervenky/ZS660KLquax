...
extern void bumper_switch(u32 val);
...
static int __init lid2_driver_probe(struct platform_device *pdev)
{
	...
	set_bit(SW_CAMERA_LENS_COVER, input->swbit);
	printk("[ASUS_LID2] %s : set ket capabilities\n", __func__);
	...
	printk("[ASUS_LID2] %s : input register ok\n", __func__);
	...
	printk("[ASUS_LID2] %s : create sysfs ok\n", __func__);
	...
	printk("[ASUS_LID2] %s : request gpio-irq ok\n", __func__);
	...
	printk("[ASUS_LID2] %s : request irq handler ok\n", __func__);
