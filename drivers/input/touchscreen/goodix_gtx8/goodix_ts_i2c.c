...
bool Reseting = false;
EXPORT_SYMBOL(Reseting);
int station_cfg_version = 0;
EXPORT_SYMBOL(station_cfg_version);
...
//				ts_info("TS_REG_DOZE_STAT read 0x%02x",r_data);
...
/**
 * goodix_send_vrcfg_command - seng vendor cmd to firmware
 *
 * @dev: pointer to device
 * @cmd: pointer to command struct which cotain command data
 * Returns 0 - succeed,<0 - failed
 */
int goodix_send_vrcfg_command(struct goodix_ts_device *dev,
		struct goodix_cfg_cmd *cmd)
{
	int ret;
	if (!cmd || !cmd->initialized)
		return -EINVAL;
/*	ts_info("goodix_send_vrcfg_command send command :");
	for(int i = 0;i < cmd->length ;i++)
	  printk(" %02X ",cmd->cmds[i]);
*/	
...
/*	if (!dev->reg.cfg_addr) {
	...
	station_cfg_version = config->data[0];
					      ...
					      u8 *config_data, u32 config_len)
{
	...
	/*disable doze mode*/
	if (dev->ic_type == IC_TYPE_NORMANDY) {
	        ts_info("IC_TYPE_NORMANDY, disable doze mode");  
		goodix_set_i2c_doze_mode(dev, false);
	}
	...
	ts_info("wait for config data ready"); 
...
//		ts_debug("EF:[%d](%d, %d)", coords->id, coords->x, coords->y);
	...
	r = 0;
	buffer[0] = 0x00;
	goodix_i2c_write_trans(dev, dev->reg.coor, buffer, 1);/*TS_REG_COORDS_BASE*/
	return r;
	...
	if (sleep_cmd.initialized) {
		...
		if (!r)
			ts_info("Chip in sleep mode");
		        ...
		        ts_info("goodix_hw_suspend complete");
	} else
...
static int goodix_hw_resume(struct goodix_ts_device *dev)
{
	...
	for (; retry > 0; retry--) {
		...
		if (dev->ic_type == IC_TYPE_NORMANDY)
			...
			for (i = 0; i < 10; i++) {
				r = goodix_i2c_read(dev, dev->reg.command, data, 1);
				if (!r && data[0] == 0xff)
					break;
				msleep(20);
			}
	...
	ts_info("goodix_hw_resume complete");
	return r;
}
...
static const struct goodix_ts_hw_ops hw_i2c_ops = {
	...
	.send_vcfg_cmd = goodix_send_vrcfg_command, //vendor config command
