...
#if 1
static void FOD_AUTH(unsigned long ptr) {
    char msg = 0;
    struct gf_dev *gf_dev = &gf;

    spin_lock(&gf_dev->timer_lock);
    if (gf_dev->timer_init) {
...
void TP_call_FOD(int id, bool down){
    ...
    pr_info("[%s] lhbm_exi2_gpio = %d  BL_val = %d lastfps = %d \n", __func__, gf_dev->lhbm_exi2_gpio, g_bl_lvl, lastFps);
    if (!gf_dev->lhbm_exi2_gpio | gf_dev->fb_black | panelOff | !g_notify_fod) {
        if (id == TP_gesture) {
            if (down)
                g_id_fod = TP_gesture;
            else 
    ...
    if (down) {
        if (id < g_id_fod) {
            ...
            pr_info("START_FOD in AOD  FG down \n");
            g_id_fod = id;
            get_tcon_cmd(0x0a, 1);
            set_fod_ir_drop(false);
