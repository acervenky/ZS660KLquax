...
//	printk("[AURA_DT] ene_8k41_read_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x\n", (addr & 0xFFFF), buf[1], buf[2]);
...
//	printk("[AURA_DT] ene_8k41_write_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x, value : 0x%x\n", (addr & 0xFFFF), buf[1], buf[2], value);
	err = i2c_write_bytes(client, buf, 3);	//set register address
	if (err !=1)
		printk("[AURA_DT] i2c_write_bytes:err %d\n", err);
...
static ssize_t vdd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret)
		return count;
	return count; //for dt the gpio is input.
	if(val>0) {
		printk("[AURA_DT] VDD set HIGH\n");
...
static ssize_t vdd_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	printk("[AURA_DT] power_gpio is invalid :set the val to -1 to show .\n");
	return snprintf(buf, PAGE_SIZE,"%d\n", val);
	/*else {
		val = regulator_is_enabled(platform_data->regulator_vdd);
		printk("[AURA_DT] regulator : %d\n", val);
		return snprintf(buf, PAGE_SIZE,"%d\n", val);
	}*/
}
...
static ssize_t led_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val>0) {
		printk("[AURA_DT] LED_EN set HIGH\n");
...
static ssize_t led_on_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val>0) {
		//printk("[AURA_DT] led_on set HIGH\n");
		//if (gDongleType == 3){
			//printk("[AURA_DT] led_on on\n");
			// set 0xF203 as 0x01 for PB0 gpio output mode enable
			g_led_on=1;
			err = ene_8k41_write_bytes(client, 0xF203, 0x01);
			if (err < 0)
		...
		//printk("[AURA_DT] led_on set LOW\n");
	//	if (gDongleType == 3){
			//printk("[AURA_DT] led_on off\n");
			// set 0xF203 as 0x00 for PB0 gpio output mode enable
			g_led_on=0;
			err = ene_8k41_write_bytes(client, 0xF203, 0x0);
			if (err < 0)
...
pinctrl:
	...
	printk("[AURA_DT] Get the active setting \n");
	// Get the active setting
	if (pdata->dongle) {
		if (gDongleType == 1){
			printk("[AURA_DT] Get inbox_aura_en \n");
		...
		printk("[AURA_DT] Get logo_5p0_en \n");
	...
	printk("[AURA_DT] set the active state\n");
...
static int ene_8k41_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//printk("[AURA_DT] check data[0]: 0x%x\n", data[0]);
...
// Set ENE 8K41 power
	if(gpio_is_valid(platform_data->power_gpio) && platform_data->dongle) {
		...
		if (err)
			printk("[AURA_DT] 8k41_power_gpio gpio_request, err %d\n", err);

		if (gDongleType == 1){
			printk("[AURA_DT] ENE 8k41 power on.\n");
			err = gpio_direction_output(platform_data->power_gpio, 1);
			if (err)
				printk("[AURA_DT] power_gpio output high, err %d\n", err);
		}else {
...
/*
	if(gpio_is_valid(platform_data->int_gpio)) {
		...
		if (err)
			printk("[AURA_DT] 8k41_int_gpio gpio_request, err %d\n", err);

		err = gpio_direction_input(platform_data->int_gpio);
		if (err)
			printk("[AURA_DT] gpio_direction_intput, err %d\n", err);
	}
*/
...
// unregister
	...
	if(gpio_is_valid(platform_data->power_gpio)) {
	// power off
		/*printk("[AURA_DT] set GPIO[%d] output low\n", platform_data->power_gpio);
		err = gpio_direction_output(platform_data->power_gpio, 0);
		if (err)
			printk("[AURA_DT] power_gpio output low, err %d\n", err);
		*/
	// free gpio
...
// free pointer
//	printk("[AURA_DT] Free platform_data\n");
//	kfree(platform_data);
...
int ene_8k41_resume(struct device *dev)
{
	...
	//("[AURA_DT] ene_8k41_resume : current_mode : 0x%x\n", g_pdata->current_mode);

	/*if(!g_pdata->current_mode){
		//printk("[AURA_DT] Enable VDD.\n");
		if ( gpio_is_valid(g_pdata->power_gpio) )
