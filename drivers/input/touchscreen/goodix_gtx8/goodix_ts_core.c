...
//		    input_sync(dev);
        ...
        /*
		if (touch_num == 1 && !touch_data->pen_down && g_notify_fod) {
	...
	if (touch_num >= 1 && touch_data->pen_down) {
		touch_num -= 1;
		if (core_data->debug)
		    ts_info("input report wit pen x:%d,y:%d,p:%d", touch_data->pen_coords[0].x, touch_data->pen_coords[0].y,touch_data->pen_coords[0].p);
...
void display_panel_off(int panel_off)
{
	  ...
	  printk("goodix touch not vaild or init fail\n"); 
	   ...
	   ts_info("phone state %d",gts_core_data->phone_call_on);
...
{        
	...
	if(atomic_read(&gts_core_data->dsi_suspend)==1){
	  ts_info("touch suspend,not enter charge mode ");
	  return;
	}
