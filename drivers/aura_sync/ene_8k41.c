...
void bumper_switch(u32 val)
{
	...
	if(val>0) {
		...
		if (err != 2)
			printk("[AURA_SYNC] bumper_en_store:err %d\n", err);
...
static ssize_t bumper_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u32 val;
	ssize_t ret;
...
static ssize_t bumper_en_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	int err = 0;
	unsigned char data[4] = {0};
	...
	err = ene_8k41_read_bytes(client, 0xF208, data);
	if (err != 2)
		printk("[AURA_SYNC] bumper_en_show:err %d\n", err);
...
static DEVICE_ATTR(bumper_enable, 0664, bumper_en_show, bumper_en_store);
