...
static ssize_t set_prota_cc_store(struct device *dev,
					  struct device_attribute *mattr,
					  const char *data, size_t count)
{
	int ret = 0;
	...
	printk("[EC_I2C] : Enter set_prota_cc_store \n");
	...
	cmd[0] =  CMD_I2C_CONTROL_PORTA_CC;
	
	if (val > 0) {
		cmd[1] = 1;
	}else {
		printk("[EC_I2C] : Error Command, do not control porta cc !!\n");
		return count;
	}
    ...
    if (g_hbm_mode == 0) {
        i2c_set_hbm(0);
    } else if (g_hbm_mode == 1) {
        i2c_set_hbm(1);
    }
    // BSP SZ --- Lydia_Wu add for Station display

    asus_dp_change_state(0,3);
    asus_dp_change_state(1,3);
	//ret = ec_i2c_write(ec_i2c_data->client,cmd,2);
...
static DEVICE_ATTR(set_prota_cc, S_IRUGO | S_IWUSR, NULL, set_prota_cc_store);
