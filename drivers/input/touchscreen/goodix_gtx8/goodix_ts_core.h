...
#define GOODIX_GLOVE_CFG_NAME "goodix_configs_glove.cfg"
#define GOODIX_TEST_CFG_NAME "goodix_config_test.cfg"
#define GOODIX_STATION_CFG_NAME "goodix_config_station.cfg"
 ...
 * @send_vcfg_cmd: send vendor command to touch device
...
struct goodix_ts_hw_ops {
	...
	int (*send_vcfg_cmd)(struct goodix_ts_device *dev, struct goodix_cfg_cmd *cmd);	
...
struct goodix_vendor_cfg{
    u8 ubLength; //command length
    u16 uwTouchLv;
    u16 uwLeaveLv; 
    u16 uwFirstFilter;
    u16 uwNormalFilter;
...
struct goodix_ts_core {
	...
	atomic_t glove_mode;
	atomic_t testcfg;
	struct goodix_vendor_cfg vendor_cfg;
	bool game_mode;
	bool atr_enable;
	bool phone_call_on;
	bool station_insert;
	atomic_t station_cfg_reload;
	atomic_t game_cfg_set;
	atomic_t rotation_set;
	struct workqueue_struct *gts_workqueue;
	struct mutex gts_suspend_mutex;
	struct workqueue_struct *gts_suspend_resume_wq;
	struct work_struct gts_resume_work;
	struct work_struct gts_suspend_work;
	struct mutex load_read_cfg_lock;
	//ASUS_BSP Beryl ---
};
...
struct goodix_ext_module_funcs {
	...
	int (*irq_event)(struct goodix_ts_core *core_data,
						struct goodix_ext_module *module);
	int (*enter_rotation)(struct goodix_ts_core *core_data,
						struct goodix_ext_module *module);
	int (*exit_rotation)(struct goodix_ts_core *core_data,
						struct goodix_ext_module *module);
	int (*charge_mode)(struct goodix_ts_core *core_data,
						struct goodix_ext_module *module);
	int (*glove_mode)(struct goodix_ts_core *core_data,
						struct goodix_ext_module *module);
	
	int (*game_cfg)(struct goodix_ts_core *core_data,
						struct goodix_ext_module *module);
};
...
static inline u16 shex_to_u16(const char *hex_buf, int size)
{
    int i;
    int base = 1;
    int value = 0;
...
#define EVT_ROTATION_SUCCESS            1
#define EVT_ROTATION_FAIL               0
#define EVT_CHANGE_MODE_SUCCESS         1 //charge_mode
#define EVT_CHANGE_MODE_CANCEL          2 //cancel enter charge mode
#define EVT_CHANGE_MODE_FAIL            0 //charge_mode
#define EVT_GLOVE_MODE_SUCCESS          1 //glove_mode
#define EVT_GLOVE_MODE_FAIL             0 //glove_mode
#define EVT_GAME_CFG_FAIL               0 //game_config
#define EVT_GAME_CFG_SUCCESS            1 //game_config
...
void ATR_touch(struct input_dev *dev, bool flag,int button);
...
void display_panel_off(int panel_off);
