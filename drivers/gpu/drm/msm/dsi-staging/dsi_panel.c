...
int lastBL = 1023; // BSP SZ +++ Lydia_Wu add for Station back light
	...
	u32 bl_lvl)
{
	...
	/* ASUS BSP Display +++ */
	if (g_enter_AOD && (bl_lvl == 273 || bl_lvl == 0))
		bl_lvl = g_alpm_bl;
...
/*
	if (g_ASUS_hwID >= ZS660KL_ER1){
	    pr_err("[Display] dsi_panel_set_lp1 panel off +++\n");
	    panel_on = false;
	    display_panel_off(1);
	}
*/
...
/*
	if (g_ASUS_hwID >= ZS660KL_ER1){
	    pr_err("[Display] dsi_panel_set_lp2 panel off +++\n");
	    display_panel_off(1);
	    panel_on = false;
	}
*/
...
int dsi_panel_set_osc(struct dsi_panel *panel)
{
	...
	if (lastFps == 120)
		...
		set_cmd[2] = g_panel_osc_p2; /* for 60 & 90 fps */
