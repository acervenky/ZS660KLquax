				   ...
				   const char *buf, size_t count)
{
	...
	case TCPC_DESC_PD_TEST:
		...
		case 9:
			...
			break;
		case 10:
			ret = tcpm_typec_disable_function(tcpc, 1);
			msleep(100);
			ret = tcpm_typec_disable_function(tcpc, 0);
			printk("tcpm_typec_disable_function off/on, ret = %d\n", ret);
			break;
		case 11:
			ret = tcpm_typec_change_role(tcpc, 3);
			printk("tcpm_typec_change_role DRP, ret = %d\n", ret);
			break;
		case 12:
			asus_request_BTM_otg_en(0);
			ret = tcpm_typec_change_role(tcpc, 1);
			printk("tcpm_typec_change_role SNK and close 5V, ret = %d\n", ret);
			break;
		default:
			break;
		}
...
static int tcpc_device_irq_enable(struct tcpc_device *tcpc)
{
	...
	schedule_delayed_work(
		&tcpc->event_init_work, msecs_to_jiffies(0*1000));
 ...
 *
 * 2.0.1_G
 *	First released PD3.0 Driver for General Platform
 */
