...
struct gesture_module {
	...
	//ASUS_BSP Beryl +++
	unsigned char zenmotion_type[ZENMOTION_LEN];
	atomic_t dclick;
	atomic_t swipeup;
	atomic_t aod_enable;
	int rotation;
		...
		const char *buf, size_t count)
{
	...
	memset(gsx_gesture->zenmotion_type, 0,ZENMOTION_LEN);
	...
	if (gesture_buf[0] == cmpchar) {
	   ...
	   if (atomic_read(&gsx_gesture->registered)) {
	       atomic_set(&gsx_gesture->zen_motion, 1);
	        for (tmp = 0;tmp < 7 ;tmp ++) {
		  gsx_gesture->zenmotion_type[tmp] = gesture_buf[tmp];
		}
