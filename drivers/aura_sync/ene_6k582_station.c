...
//	printk("[AURA_POGO] ene_6k582_read_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x\n", (addr & 0xFFFF), buf[1], buf[2]);
...
//	printk("[AURA_POGO] ene_6k582_write_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x, value : 0x%x\n", (addr & 0xFFFF), buf[1], buf[2], value);
	err = i2c_write_bytes(client, buf, 3);	//set register address
	if (err !=1)
		printk("[AURA_POGO] i2c_write_bytes:err %d\n", err);
...
// (5) Program
	printk("[AURA_POGO] Program FW from 0x0000 to 0X6B80 =====\n");
	...
	//program checksum
	printk("[AURA_POGO] set addr : 0x6FF0\n");
	...
	/*for(i=1;i<=16;i++){
		printk("[AURA_POGO] checksum buf[%d] = %x\n",i,buf[i]);
	}*/
...
static ssize_t red1_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//printk("[AURA_POGO] red1_pwm_store.\n");
...
static ssize_t red1_pwm_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (err != 2)
		printk("[AURA_POGO] red1_pwm_show:err %d\n", err);
...
static ssize_t green_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&g_pdata->ene_mutex);
	//printk("[AURA_POGO] client->addr : 0x%x,  reg_val : 0x%x.\n", client->addr, reg_val);
	//printk("[AURA_POGO] first led green tmp %d\n", tmp);
	g_green=tmp;
	err = ene_6k582_write_bytes(client, 0x8406, tmp);
	if (err !=1)
		printk("[AURA_POGO] ene_6k582_write_bytes:err %d\n", err);
	//printk("[AURA_POGO] second led green tmp %d\n", tmp);
...
static ssize_t green1_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//printk("[AURA_POGO] green1_pwm_store.\n");
...
static ssize_t green1_pwm_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (err != 2)
		printk("[AURA_POGO] green1_pwm_show:err %d\n", err);
...
static ssize_t blue_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&g_pdata->ene_mutex);
	//printk("[AURA_POGO] client->addr : 0x%x,  reg_val : 0x%x.\n", client->addr, reg_val);
	//printk("[AURA_POGO] first led blue tmp %d\n", tmp);
	g_blue=tmp;
	err = ene_6k582_write_bytes(client, 0x8408, tmp);
	if (err !=1)
		printk("[AURA_POGO] ene_6k582_write_bytes:err %d\n", err);
	//printk("[AURA_POGO] second led blue tmp %d\n", tmp);
...
static ssize_t blue1_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//printk("[AURA_POGO] blue1_pwm_store.\n");
...
static ssize_t blue1_pwm_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (err != 2)
		printk("[AURA_POGO] blue1_pwm_show:err %d\n", err);
...
static ssize_t apply_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&g_pdata->ene_mutex);
	if (val > 0){
			printk("[AURA_POGO] Send apply. RGB:%d %d %d, mode:%d, speed:%d, led_on:%d, led2_on:%d\n", g_red, g_green, g_blue, g_mode, g_speed, g_led_on, g_led2_on);
...
static ssize_t mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	g_mode=val;
	 switch(val){
	 	...
	 	case 1: //static
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] mode: 0x%x  0x8401-->0x03.\n",  val);
			...
			//printk("[AURA_POGO] mode: 0x%x  0x840A-->0x01.\n",  val);
			...
			//printk("[AURA_POGO] mode: 0x%x  0x840B-->0x00.\n",  val);
			...
			//printk("[AURA_POGO] mode: 0x%x  0x840F-->0xE4.\n",  val);
...
static ssize_t fw_update_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(fw_size<=0){
		printk("[AURA_POGO] fwsize = %d, return\n",fw_size);
		return count;
	}
	...
	if(err){
		printk("[AURA_POGO] ene_UpdateFirmware failede for the first time, and we will retry a time, err %d\n", err);
		err = ene_UpdateFirmware(client, fw_buf);
		if(err){
			printk("[AURA_POGO] ene_UpdateFirmware failed at the second time, err %d\n", err);
		}
...
static ssize_t set_frame(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&g_pdata->ene_mutex);
	printk("[AURA_POGO] set_frame client->addr : 0x%x,  val : 0x%x.\n", client->addr, val);
...
static ssize_t read_reg_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (err != 2)
		printk("[AURA_POGO] read_reg_show:err %d\n", err);
...
static ssize_t read_reg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[AURA_POGO] read_reg_store.\n");
	sscanf(buf, "%x", &addr_reg);
	printk("[AURA_POGO] addr=0x%x.\n",addr_reg);
	return count;
}
...
static ssize_t reg_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[AURA_POGO] reg_debug_store.\n");

	sscanf(buf, "%x %x ", &addr, &val);


	printk("[AURA_POGO] addr=0x%x, val=%x .\n",addr,val);
...
static ssize_t mode2_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[AURA_POGO] mode2_store mode2=%d buf=%s ntokens=%d\n",mode2,buf,ntokens);
	if(ntokens > 6) 
	{
		printk("[AURA_POGO] mode2_store,wrong input,too many ntokens\n");
		mode2_state=-1;
		return count;
	}
	...
	if(rgb_num != ntokens*3){
		printk("[AURA_POGO] mode2_store,wrong input,rgb_num != ntokens*3\n");
		mode2_state=-1;
		return count;
	}
	...
	switch(mode2){
		...
		case 1: //6 color rainbow
			if(ntokens!=6){
				printk("[AURA_POGO] mode2_store,wrong input. ntokensis not 6\n");
				mode2_state=-1;
				return count;
			}
			//printk("[AURA_POGO] mode2: 0x%x - 6 colors rainbow \n",  mode2);
			mutex_lock(&g_pdata->ene_mutex);
			for(i=0;i<6;i++){
				addr =  0x8420 + i;
		...
		case 2:  //static
			if(ntokens!=2){
				printk("[AURA_POGO] mode2_store,wrong input. ntokensis not 2\n");
				mode2_state=-1;
				return count;
			}
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] client->addr : 0x%x,  mode2 : 0x%x -- static.\n", client->addr, mode2);
			for(i=0;i<2;i++){
				addr = 0x8404+i;
				err = ene_6k582_write_bytes(client, addr, rgb[i*3]);
				if (err !=1){
		...
		case 3:  //breath at the same time
			...
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] client->addr : 0x%x,  mode2 : 0x%x -- breath at the same time.\n", client->addr, mode2);
			for(i=0;i<2;i++){
				addr = 0x8404+i;
				err = ene_6k582_write_bytes(client, addr, rgb[i*3]);
				if (err !=1){
		...
		case 4:  //breath diff
			...
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] client->addr : 0x%x,  mode2 : 0x%x -- breath diff.\n", client->addr, mode2);
			for(i=0;i<2;i++){
				addr = 0x8404+i;
				err = ene_6k582_write_bytes(client, addr, rgb[i*3]);
				if (err !=1){
		...
		case 5: //breath one led
			...
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] client->addr : 0x%x,  mode2 : 0x%x -- breath one led.\n", client->addr, mode2);
			for(i=0;i<2;i++){
				addr = 0x8404+i;
				err = ene_6k582_write_bytes(client, addr, rgb[i*3]);
				if (err !=1){
		...
		case 6: //commet
			...
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] client->addr : 0x%x,  mode2 : 0x%x -- commet.\n", client->addr, mode2);
			for(i=0;i<2;i++){
				addr = 0x8404+i;
				err = ene_6k582_write_bytes(client, addr, rgb[i*3]);
				if (err !=1){
		...
		case 7: //flash and dash
			...
			mutex_lock(&g_pdata->ene_mutex);
			//printk("[AURA_POGO] client->addr : 0x%x,  mode2 : 0x%x -- flash and dash.\n", client->addr, mode2);
			for(i=0;i<2;i++){
				addr = 0x8404+i;
				err = ene_6k582_write_bytes(client, addr, rgb[i*3]);
				if (err !=1){
...
static ssize_t led_on_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (err != 2){
		printk("[AURA_POGO] led_on_store:err %d\n", err);
	}
	mutex_unlock(&g_pdata->ene_mutex);
	if(val==1){  //ON
		g_led_on=1;
...
static ssize_t led_on_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	err = ene_6k582_read_bytes(client, 0x840c, data);
	if (err != 2){
		printk("[AURA_POGO] led_on_show:err %d\n", err);
	}
	mutex_unlock(&g_pdata->ene_mutex);
	//printk("[AURA_POGO] led_on_show,value = 0x%x\n");
	if((data[0]&0x03) == 0x03){
		val = 1;
...
static ssize_t led2_on_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	mutex_unlock(&g_pdata->ene_mutex);
	//printk("[AURA_POGO] led2_on_show,value = 0x%x\n");
	if((data[0]&0x0C) == 0xC){
		val = 1;
...
static ssize_t sleep_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val==1){  //stop mode
		...
		if (err != 2){
			printk("[AURA_POGO] sleep_show:err %d\n", err);
		}
		mutex_unlock(&g_pdata->ene_mutex);
		if(data[0]==0xAA){
			printk("[AURA_POGO] ene_6k582 resume ok\n");
		}else{
		mutex_lock(&g_pdata->ene_mutex);
		err = ene_6k582_write_bytes(client, 0XF010, 0x05);
		if (err !=1)
...
static ssize_t sleep_time_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[AURA_POGO] sleep_time_store data[1]=%x data[0]=%x delay_time=%d\n",data[1],data[0],delay_time);
	//set delay time
	mutex_lock(&g_pdata->ene_mutex);
...
static int ene_6k582_fb_callback(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	printk("[AURA_POGO] go to the ene_6k582_fb_callback value = %d msm_drm_display_id = %d\n", (int)val, evdata->id);
	...
	switch (blank) {
		case MSM_DRM_BLANK_POWERDOWN:
			printk("[AURA_POGO] ene_6k582_suspend : current_mode : 0x%x\n", g_pdata->current_mode);
		...
		case MSM_DRM_BLANK_UNBLANK:
			printk("[AURA_POGO] ene_6k582_resume : current_mode : 0x%x\n", g_pdata->current_mode);
			/*if(!platform_data->current_mode){
				err = asus_wait4hid();
				if (err)
...
static int ene_6k582_parse_dt(struct device *dev, struct ene_6k582_platform_data *pdata)
{
	...
	printk("[AURA_POGO] ene_6k582_parse_dt\n");
...
static int ene_6k582_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[AURA_POGO] ene_6k582_probe.\n");
...
// Parse platform data from dtsi
	...
	if (err) {
		printk("[AURA_POGO] ene_6k582_parse_dt get fail !!!\n");
		return -ENOMEM;
	}
...
//	hid_to_gpio_set(0x3F, 1);
	...
	if (err<0) {
		printk("[AURA_POGO] ec_i2c_set_gpio err<0 ;IC power on fail!!!\n");
		//return -ENOMEM;
	}
	msleep(500);
	printk("[AURA_POGO] ==========\n");
...
// Setting PWM inverse
	...
	if (err != 2)
		printk("[AURA_POGO] ene_6k582_read_bytes:err %d\n", err);
...
// Set global variable
	...
	printk("[AURA_POGO] ene_6k582_probe done.\n");
	///kfree(fw_buf);
	return 0;

unled:
	aura_sync_unregister(platform_data);
	printk("[AURA_POGO] ENE 6k852 power off.\n");
	return err;

/*exit_check_functionality_failed:
	printk("[AURA_POGO] ene_6k582_probe fail !!!\n");
...
int ene_6k582_suspend(struct device *dev)
{
	...
	printk("[AURA_POGO] ene_6k582_suspend , delay_time=%d\n",delay_time);
	if(platform_data->current_mode != 0){
		printk("[AURA_POGO] ene_6k582_suspend it is not mode 0\n");
		return 0;
	}
	//set delay time
	mutex_lock(&g_pdata->ene_mutex);
...
int ene_6k582_resume(struct device *dev)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct ene_6k582_platform_data *platform_data = i2c_get_clientdata(client);
	printk("[AURA_POGO] ene_6k582_resume\n");
	if(platform_data->current_mode){
		printk("[AURA_POGO] ene_6k582_resume it is not mode 0\n");
		return 0;
	}
	mutex_lock(&g_pdata->ene_mutex);
	err = ene_6k582_write_bytes(client, 0XF010, 0x05);
	...
	mutex_unlock(&g_pdata->ene_mutex);
	if(data[0]==0xAA){
		printk("[AURA_POGO] ene_6k582_resume  ok\n");
	}else{
		mutex_lock(&g_pdata->ene_mutex);
		err = ene_6k582_write_bytes(client, 0XF010, 0x05);
		if (err !=1)
...
static int __init ene_6k582_bus_init(void)
{
	...
	if (ret)
		printk("[AURA_POGO] ENE 6k852 driver int failed.\n");
	else
		printk("[AURA_POGO] ENE 6k852 driver int success.\n");
